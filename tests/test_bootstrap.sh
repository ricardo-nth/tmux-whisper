...
# Build a local archive that mirrors GitHub codeload structure: dictate-cli-<ref>/
ARCHIVE_ROOT="$TMP_ROOT/archive-root"
SNAPSHOT_DIR="$ARCHIVE_ROOT/dictate-cli-main"
...
tar -czf "$TMP_ROOT/dictate-cli-main.tar.gz" -C "$ARCHIVE_ROOT" dictate-cli-main

HOME="$TMP_ROOT/home"
export HOME
mkdir -p "$HOME"

ARCHIVE_URL="file://$TMP_ROOT/dictate-cli-main.tar.gz"
...
assert_file "$HOME/.local/bin/dictate"
