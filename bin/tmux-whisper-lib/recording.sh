...
terminate_recording_pid() {
  ...
  kill -0 "$target_pid" 2>/dev/null && alive_before_grace="1"

  if keep_logs_enabled; then
    printf "[%s] stop_recording: pid=%s grace_ms=%s int_wait_ms=%s term_wait_ms=%s stop_begin_ms=%s alive_before_grace=%s\n" \
      "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" "$target_pid" "$stop_grace_ms" \
      "$(( int_wait_attempts * 50 ))" "$(( term_wait_attempts * 50 ))" "$stop_begin_ms" "$alive_before_grace" \
  ...
  alive_after_grace="0"
  kill -0 "$target_pid" 2>/dev/null && alive_after_grace="1"
  if keep_logs_enabled; then
